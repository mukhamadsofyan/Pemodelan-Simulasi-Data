...
Original file is located at
    https://colab.research.google.com/drive/1rCo9PoX45oGQgynja_njrMtKykE7V8Fc

## Identitas

- **Nama:** Mukhamad Sofyan  
- **NIM:** 202310370311135  
- **Kelas:** Pemodelan & Simulasi Data C

# Simulasi Sistem Antrian Layanan Konseling Mahasiswa Menggunakan Agent Based Modeling untuk Analisis Waktu Tunggu dan Efisiensi Sistem

## Revisi Progress

Pada progress sebelumnya, saya sudah jadi dashboard tapi masih menggunakan jumlah data mahasiswa sebanyak 200 data. Berdasarkan revisi yang diberikan, jumlah data pada simulasi telah ditingkatkan menjadi minimal 1000 data mahasiswa agar proses simulasi, analisis antrean, serta pengujian performa sistem menjadi lebih realistis dan mendekati kondisi sebenarnya.

## Install library
Cell ini memasang library yang dibutuhkan untuk menjalankan aplikasi Streamlit di Colab.
"""

!pip install streamlit fpdf -q
...
"""## Engine Simulasi Agent Based Modeling

Mahasiswa dimodelkan sebagai `StudentAgent`, konselor sebagai `CounselorAgent`, dan sistem antrian sebagai `CounselingEnvironment`. Setiap mahasiswa memiliki perilaku individual seperti peningkatan stres saat menunggu, toleransi waktu tunggu, prioritas risiko, dan perubahan stres setelah CBT. Konselor memiliki perilaku pelayanan, idle time, beban kerja, dan fatigue yang memengaruhi durasi layanan. Interaksi antar-agent inilah yang menghasilkan waktu tunggu dan efisiensi sistem.
...
Membuat laporan hasil simulasi dalam format CSV, PDF, atau TXT. Isi laporan menggunakan bahasa Indonesia dan memuat ringkasan simulasi, indikator kinerja, serta rekomendasi sistem berdasarkan hasil ABM.

"""

# Commented out IPython magic to ensure Python compatibility.
# %%writefile export_utils.py
...
membuat visualisasi hasil simulasi dalam bentuk grafik SVG tanpa menggunakan library visualisasi eksternal. Grafik yang dihasilkan meliputi line chart, bar chart, dan histogram untuk membantu analisis waiting time, workload counselor, serta perubahan tingkat stres mahasiswa setelah CBT.
"""

# Commented out IPython magic to ensure Python compatibility.
# %%writefile svg_charts.py
# import numpy as np
# 
# # ══════════════════════════════════════════════
...
Membuat komponen tampilan tambahan pada aplikasi Streamlit, seperti kartu KPI, judul section, dan panel grafik. Tujuannya agar tampilan dashboard lebih rapi, konsisten, dan mudah dibaca saat menampilkan hasil simulasi.
"""

# Commented out IPython magic to ensure Python compatibility.
# %%writefile ui_helpers.py
# import streamlit as st
# 
# # ══════════════════════════════════════════════
# # UI HELPERS
...
Cell ini digunakan untuk menjalankan aplikasi utama Streamlit yang menggabungkan keseluruhan, mulai dari login, pengaturan parameter simulasi, upload dataset, eksekusi simulasi, visualisasi grafik, tabel hasil, export laporan, Monte Carlo analysis, hingga rekomendasi sistem berdasarkan hasil antrean konseling.
"""
...
# from sim_engine import CounselingQueueSimulation, generate_dataset
# from export_utils import try_fpdf
# from svg_charts import make_svg_line, make_svg_bar, make_svg_hist
# from ui_helpers import kpi, sec, chart
# 
# st.set_page_config(
...
"""## Streamlit Deployment & Cloudflare Tunnel

Jadi disini karena punya saya sudah jadi dashboard maka testingnya melalui trycloudflare.com.

### Dashboard Demo
https://sean-tricks-award-western.trycloudflare.com
...
# Streamlit dengan Cloudflare Tunnel
