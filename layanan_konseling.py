...
class StudentAgent(mesa.Agent):
    ...
    Agen mahasiswa.
    Atribut utama:
    - urgency_level: 1=rendah, 2=sedang, 3=tinggi
    - waiting_time: waktu tunggu di antrian
    - patience_limit: batas kesabaran sebelum keluar
    - service_time_needed: total waktu pelayanan yang dibutuhkan
    - status: waiting, in_service, done, dropout
    """
    ...
    def __init__(self, unique_id, model, arrival_time):
        ...
        self.service_time_needed = self._generate_service_time()
        self.status = "waiting"
    ...
    def step(self):
        # Logika utama mahasiswa diatur model dan counselor
        pass
...
class CounselorAgent(mesa.Agent):
    """
    Agen konselor.
    Setiap konselor melayani 1 mahasiswa pada satu waktu.
    """
    ...
    def assign_student(self, student):
        adjusted_service_time = max(
            1, round(student.service_time_needed * self.service_speed_factor)
        )
    ...
    def release_student(self):
        ...
        self.model.system_times.append(student.end_service_time - student.arrival_time)
...
class CounselingQueueModel(mesa.Model):
    """
    Model antrian layanan konseling mahasiswa berbasis ABM dengan Mesa.
    """
    ...
    def __init__(
        ...
        # Scheduler klasik Mesa
        self.schedule = mesa.time.BaseScheduler(self)

        # Counter unique_id agent
        ...
        self.datacollector = mesa.DataCollector(
            model_reporters={
                ...
                "current_arrival_prob": lambda m: m.get_arrival_probability(),
            }
        )
    ...
    def generate_new_students(self):
        """
        Bisa menghasilkan 0, 1, atau 2 mahasiswa per step.
        """
    ...
    def sort_priority_queue(self):
        if self.queue_policy == "PRIORITY" and len(self.waiting_queue) > 1:
            ...
            self.waiting_queue = deque(sorted_list)
    ...
    def update_waiting_students(self):
        ...
        while self.waiting_queue:
            ...
            if student.waiting_time > student.patience_limit:
                ...
                # Hapus dari scheduler agar tidak terus ikut step
    ...
    def cleanup_finished_students(self):
        """
        Hapus mahasiswa yang sudah selesai dari scheduler supaya lebih rapi.
        """
        to_remove = []
    ...
    def step(self):
        ...
        # 2. Assign ke konselor idle
        self.assign_students_to_counselors()

        # 3. Jalankan semua agen
        self.schedule.step()

        # 4. Update mahasiswa yang masih menunggu
        self.update_waiting_students()

        # 5. Assign lagi bila ada konselor yang baru kosong
        self.assign_students_to_counselors()

        # 6. Bersihkan agen mahasiswa yang sudah selesai
...
if __name__ == "__main__":
    model = CounselingQueueModel(
        ...
        max_steps=480,
        base_arrival_prob=0.35,
        queue_policy="FIFO",   # bisa diganti "PRIORITY"
        random_seed=42,
    )
